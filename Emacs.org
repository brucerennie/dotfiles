...
#+begin_src emacs-lisp :tangle emacs/init.el
  ...
  ;; Don't let C-z (or C-x C-z) suspend the frame. The Termux on-screen
  ;; keyboard's "undo" button sends C-z, which suspends the emacsclient
  ;; running inside a tmux window and looks like the daemon died.
...
When Emacs runs as a =--daemon=, it creates an invisible bookkeeping frame (often shown as =F1= with no TTY).  That hidden frame keeps claiming minibuffer ownership, so when an =emacsclient -t= frame attaches and I hit =M-x=, the minibuffer can end up on the hidden frame while my keystrokes go to the TTY frame — classic "stuck M-x" symptom where letters all register as "undefined" and =C-g= won't break out.

The fix is to delete the daemon's hidden frames whenever a real client attaches, leaving only visible / TTY frames.

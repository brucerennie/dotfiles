...
(when (daemonp)
  (add-hook 'server-after-make-frame-hook
            (lambda ()
              ;; The daemon's initial hidden frame has no =client= AND no
              ;; =tty-type=.  That is the one we want to delete.  Do NOT
              ;; rely on =frame-visible-p= -- the daemon frame reports as
              ;; visible even though it has no display, so the obvious
              ;; check silently skips it.
              ;;
              ;; Defensive: never delete the just-made frame, which is
              ;; =(selected-frame)= at hook-fire time.
              (let ((just-made (selected-frame)))
                (dolist (f (frame-list))
                  (when (and (not (eq f just-made))
                             (null (frame-parameter f 'client))
                             (null (frame-parameter f 'tty-type)))
                    (condition-case err
                        (delete-frame f t)
                      (error
